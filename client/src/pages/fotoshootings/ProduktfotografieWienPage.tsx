      ...
      <div className="min-h-screen bg-white">
        ...
        <section className="py-16 bg-gray-50">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-3xl font-bold text-center mb-12">{language === 'de' ? 'Häufige Fragen (FAQ)' : 'Frequently Asked Questions (FAQ)'}</h2>
            <div className="space-y-6">
              <div className="bg-white rounded-xl p-6 shadow-sm"><h3 className="font-bold text-lg text-gray-900 mb-2">{language === 'de' ? 'Könnt ihr stark spiegelnde Produkte (Chrom/Glas)?' : 'Can you handle highly reflective products (chrome/glass)?'}</h3><p className="text-gray-600">{language === 'de' ? 'Ja. Mit Flagging, Polarizer, Streifenlicht und kontrollierten Specular Highlights.' : 'Yes. With flagging, polarizer, strip lighting and controlled specular highlights.'}</p></div>
              <div className="bg-white rounded-xl p-6 shadow-sm"><h3 className="font-bold text-lg text-gray-900 mb-2">{language === 'de' ? 'Baut ihr Sets/Untergründe?' : 'Do you build sets/backgrounds?'}</h3><p className="text-gray-600">{language === 'de' ? 'Ja. Acryl (klar/schwarz), Stein, Holz, Paper, Plexi – individuell abgestimmt.' : 'Yes. Acrylic (clear/black), stone, wood, paper, plexiglass – individually tailored.'}</p></div>
              <div className="bg-white rounded-xl p-6 shadow-sm"><h3 className="font-bold text-lg text-gray-900 mb-2">{language === 'de' ? 'Brauchen wir Muster oder Render?' : 'Do we need samples or renders?'}</h3><p className="text-gray-600">{language === 'de' ? 'Echte Muster liefern realistische Highlights/Texturen. Render nur als Ergänzung.' : 'Real samples give realistic highlights/textures. Renders only as a supplement.'}</p></div>
              <div className="bg-white rounded-xl p-6 shadow-sm"><h3 className="font-bold text-lg text-gray-900 mb-2">{language === 'de' ? 'Wie viele Dateien je Motiv?' : 'How many files per motif?'}</h3><p className="text-gray-600">{language === 'de' ? 'Standard: 1 Hauptlook, optional Zusatzwinkel/Detail. Benennung nach SKU/Variante.' : 'Standard: 1 main look, optional extra angles/details. Named by SKU/variant.'}</p></div>
            </div>
          </div>
        </section>
