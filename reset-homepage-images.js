...
(async () => {
  try {
    console.log('🗑️  Dropping old homepage_images table...');
    await sql`DROP TABLE IF EXISTS homepage_images CASCADE`;
    console.log('✅ Dropped');
    
    console.log('🔧 Creating new homepage_images table...');
    await sql`
    ...
    await sql`CREATE INDEX idx_homepage_images_section ON homepage_images(section)`;
    await sql`CREATE INDEX idx_homepage_images_active ON homepage_images(is_active)`;
    
    console.log('✅ Table created');
    
    // Seed with Backblaze URLs
    ...
    for (const img of backblazeImages) {
      ...
      `;
      console.log(`✅ ${img.section}: ${img.title}`);
    }
    ...
    console.log('\n📊 Database: Neon ✅');
    console.log('📊 Image storage: Backblaze B2 ✅');
