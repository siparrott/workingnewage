...
interface OnboardingStats {
  pagesScanned?: number;
  fixFirstItemsCount?: number;
  fixFirstItemsCompleted?: number;
...
class HubIntegration {
  ...
  private licenseData: LicenseValidationResult['license'] | null = null;
  private workspaceData: LicenseValidationResult['workspace'] | null = null;
  private onboardingData: LicenseValidationResult['onboarding'] | null = null;
  private lastValidation: Date | null = null;
  
  // Cache duration for license validation (1 hour)
  private readonly VALIDATION_CACHE_MS = 60 * 60 * 1000;
   ...
   */
  async validateLicense(force: boolean = false): Promise<LicenseValidationResult> {
    ...
    try {
      ...
      const response = await fetch(`${HUB_URL}/api/license/validate`, {
        method: 'POST',
        headers: {
   ...
   */
  async reportProgress(step: string, stats?: OnboardingStats): Promise<boolean> {
    if (!this.isConfigured()) {
    ...
    try {
      const response = await fetch(
        `${HUB_URL}/api/workspace/${WORKSPACE_ID}/onboarding/progress`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
...
export function licenseMiddleware() {
  return async (req: any, res: any, next: any) => {
    ...
    const result = await hubIntegration.validateLicense();
...
export function setupModeMiddleware() {
  ...
  return (req: any, res: any, next: any) => {
    if (!hubIntegration.isSetupMode()) {
      return next();
    }
    
    // Check if path is allowed during setup
    const isAllowed = ALLOWED_DURING_SETUP.some(
