...
router.post('/start-workflow', async (req, res) => {
  try {
    ...
    const workflowResult = await pool.query(
      'SELECT * FROM workflow_instances WHERE id = $1 AND status = $2',
      [workflowId, 'active']
    );
    ...
    const executionResult = await pool.query(
      ...
      RETURNING *`,
      [workflowId, clientId, 'in_progress', context || {}]
    );
...
router.get('/execution/:id', async (req, res) => {
  try {
    ...
    // Get step executions
    const stepExecutionsResult = await pool.query(
      `SELECT wse.*, ws.step_name, ws.step_type 
...
router.post('/pause-workflow', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE workflow_instances SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING *',
      ['paused', workflowId]
    );
...
router.post('/cancel-workflow', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE workflow_instances SET status = $1, completed_at = NOW(), updated_at = NOW() WHERE id = $2 RETURNING *',
    ...
    await pool.query(
      ...
      ['cancelled', workflowId, 'in_progress']
    );
