...
export class DATEVCSVAdapter implements ExportAdapter {
  readonly name = 'DATEV CSV (Buchungsstapel)';
  readonly version = '1.0.0';
  readonly profile = 'datev_csv' as const;
  readonly description = 'DATEV-compatible CSV export for DACH accountants';
   ...
   */
  private generateBuchungsstapel(data: CLSExportData): string {
    const lines: string[] = [];
   ...
   */
  private generateKontenbeschriftungen(data: CLSExportData): string {
    const lines: string[] = [];
   ...
   */
  private getBUSchluessel(taxRate: number, reverseCharge: boolean): string {
    if (reverseCharge) return '93'; // Reverse charge
    if (taxRate === 0.20) return '9';  // 20% VAT
    if (taxRate === 0.13) return '2';  // 13% VAT
    if (taxRate === 0.10) return '3';  // 10% VAT
    if (taxRate === 0.00) return '40'; // Tax-free
   ...
   */
  private formatDATEVDate(dateStr: string): string {
   ...
   */
  private isValidDATEVDate(dateStr: string): boolean {
    try {
      const date = new Date(dateStr);
   ...
   */
  private datevRound(value: number): number {
