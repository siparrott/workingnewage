...
export class XeroCSVAdapter implements ExportAdapter {
  readonly name = 'Xero CSV';
  readonly version = '1.0.0';
  readonly profile = 'csv_xero' as const;
  readonly description = 'Xero-compatible CSV export for invoices and payments';
   ...
   * Generate Sales CSV
   */
  private generateSalesCSV(data: CLSExportData): string {
    const headers = [
      '*ContactName',
      'EmailAddress',
      'POAddressLine1',
   ...
   * Generate Tax Summary CSV
   */
  private generateTaxSummaryCSV(data: CLSExportData): string {
    const headers = [
      'TaxType',
      'TaxRate',
      'NetAmount',
    ...
    // Group by tax rate
    const taxGroups = new Map<string, { net: number; tax: number; gross: number; count: number }>();
   ...
   */
  private getXeroTaxType(rate: number): string {
    if (rate === 0) return 'Zero Rated';
    if (rate === 0.10) return 'Tax on Sales 10%';
    if (rate === 0.13) return 'Tax on Sales 13%';
    if (rate === 0.20) return 'Tax on Sales 20%';
