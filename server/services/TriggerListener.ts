...
class TriggerListener extends EventEmitter {
  ...
  private setupEventListeners() {
    ...
    this.on('new_client', async (data) => {
      ...
      await workflowExecutionService.handleTrigger('new_client', data);
    });
    ...
    this.on('booking_confirmed', async (data) => {
      ...
      await workflowExecutionService.handleTrigger('booking_confirmed', data);
    });
    ...
    this.on('invoice_sent', async (data) => {
      ...
      await workflowExecutionService.handleTrigger('invoice_sent', data);
    });
    ...
    this.on('gallery_uploaded', async (data) => {
      ...
      await workflowExecutionService.handleTrigger('gallery_uploaded', data);
    });
   ...
   */
  trigger(type: TriggerType, data: any): void {
  ...
  triggerNewClient(clientId: string): void {
    this.trigger('new_client', { clientId });
  }

  triggerBookingConfirmed(bookingId: string, clientId?: string): void {
    this.trigger('booking_confirmed', { bookingId, clientId });
  }

  triggerInvoiceSent(invoiceId: string, clientId?: string): void {
    this.trigger('invoice_sent', { invoiceId, clientId });
  }

  triggerGalleryUploaded(galleryId: string, clientId?: string): void {
    this.trigger('gallery_uploaded', { galleryId, clientId });
  }
}

// Export singleton instance
export const triggerListener = new TriggerListener();
