...
### July 14, 2026
- **Automations email editor** — the "Write" (visual) editor did a lossy HTML→text→HTML round-trip: ragged blank-line pagination, and any edit silently destroyed the CTA button + `{{questionnaireLink}}` anchor. Now lossless — the button survives as a `[label](url)` token both ways, blank lines are normalised, and Preview renders the exact HTML that will be sent.
