...
- **Storage note (found during this work):** the live upload `S3Client` (`server/routes.ts:1330`, `server/routes/files.ts:16`) reads **`process.env.AWS_*` only** (Backblaze default endpoint) — it does NOT read the onboarding wizard's DB storage config (`studio_integrations`). So a tenant who configures storage via the wizard would NOT have uploads work unless the AWS_* env is also set. Same class of bug as the SMS bridge; bridging the S3 client through `config-reader` is a recommended onboarding hardening item.
