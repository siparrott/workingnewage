...
const params = z.object({
  startDate: z.string().optional().describe("Filter sales from this date (YYYY-MM-DD)"),
  endDate: z.string().optional().describe("Filter sales until this date (YYYY-MM-DD)"),
  paymentStatus: z.enum(["pending", "paid", "refunded", "all"]).optional().default("all").describe("Filter by payment status"),
  limit: z.number().int().min(1).max(500).default(50).optional().describe("Maximum number of records to return")
});
