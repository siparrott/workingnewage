...
const params = z.object({
  ...
  clientName: z.string().optional().describe("Find session by client name"),
  upcoming: z.boolean().optional().describe("If true, get the next upcoming session"),
  status: z.enum(["scheduled", "in_progress", "completed", "cancelled", "any"]).default("any").optional().describe("Filter by session status")
});
...
Returns: Full session details with related information`,
  ...
  handler: async (ctx: ToolContext, args: z.infer<typeof params>) => {
    try {
      ...
      const communications = commsResult.rows.map((c: any) => ({
      ...
      return {
        ...
        equipment: {
          ...
          rental_count: equipment.filter((e: any) => e.rental).length,
          total_rental_cost: equipment.reduce((sum: number, e: any) => sum + (e.rental ? e.cost : 0), 0),
          items: equipment
        },
