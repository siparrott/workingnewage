...
async function setupDemoMode() {
    ...
    try {
        ...
        let demoUser = await db_1.db.select()
            .from(schema_1.users)
            .where((0, drizzle_orm_1.eq)(schema_1.users.email, DEMO_USER_EMAIL))
            .limit(1);
        if (demoUser.length === 0) {
            console.log('   Creating demo user...');
            [demoUser[0]] = await db_1.db.insert(schema_1.users).values({
                id: DEMO_USER_ID,
        ...
        let demoSubscription = await db_1.db.select()
            .from(schema_1.storageSubscriptions)
            .where((0, drizzle_orm_1.eq)(schema_1.storageSubscriptions.userId, demoUser[0].id))
            .limit(1);
        if (demoSubscription.length === 0) {
            ...
            [demoSubscription[0]] = await db_1.db.insert(schema_1.storageSubscriptions).values({
        ...
        const existingUsage = await db_1.db.select()
            .from(schema_1.storageUsage)
            .where((0, drizzle_orm_1.eq)(schema_1.storageUsage.subscriptionId, demoSubscription[0].id))
            .limit(1);
