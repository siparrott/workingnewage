...
function extractTitleAndLinks(html, baseUrl) {
    ...
    try {
        ...
        urls.forEach((raw) => {
            try {
                resolved.push(new URL(raw, baseUrl).toString());
            }
            catch { }
        });
        out.links = resolved;
...
async function findSitemaps(siteUrl) {
    ...
    try {
        ...
        if (r.ok) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (m && m[1]) {
                    try {
                        sites.add(new URL(m[1].trim(), robotsUrl).toString());
                    }
                    catch { }
                }
            }
    ...
    try {
        sites.add(new URL("/sitemap.xml", siteUrl).toString());
    }
    catch { }
    try {
        sites.add(new URL("/sitemap_index.xml", siteUrl).toString());
    }
    catch { }
...
router.post("/start", async (req, res) => {
    try {
        ...
        if (!startUrl)
            return res.status(400).json({ error: "start_url required" });
        const custEmail = email || customer_email || null;
        const ins = await db_1.pool.query(`INSERT INTO onboarding_sessions(customer_email, stripe_session_id, start_url, status, crawl_status)
       VALUES ($1, $2, $3, 'started', 'pending') RETURNING id`, [custEmail, stripe_session_id || null, startUrl]);
        const session_id = ins.rows[0].id;
        const job = await db_1.pool.query(`INSERT INTO crawl_jobs(onboarding_session_id, seed_url, status)
       VALUES ($1, $2, 'pending') RETURNING id`, [session_id, startUrl]);
...
router.post("/run-crawl", async (req, res) => {
    try {
        ...
        const s = await db_1.pool.query(`SELECT id, start_url FROM onboarding_sessions WHERE id = $1 LIMIT 1`, [sid]);
        if (!s.rows.length)
            return res.status(404).json({ error: "session not found" });
        const startUrl = s.rows[0].start_url;
        const j = await db_1.pool.query(`SELECT id, status FROM crawl_jobs WHERE onboarding_session_id = $1 ORDER BY created_at DESC LIMIT 1`, [sid]);
        if (!j.rows.length)
            return res.status(404).json({ error: "crawl job not found" });
        const jobId = j.rows[0].id;
        await db_1.pool.query(`UPDATE crawl_jobs SET status = 'running', started_at = now(), error = NULL WHERE id = $1`, [jobId]);
        ...
        while (queue.length && crawled < maxPages) {
            ...
            try {
                ...
                await db_1.pool.query(`INSERT INTO website_pages(crawl_job_id, url, status, http_status, content_type, title, html, text_content, links, assets, meta)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9::jsonb, $10::jsonb, $11::jsonb)`, [
                ...
                await db_1.pool.query(`INSERT INTO website_pages(crawl_job_id, url, status, http_status, content_type, title, html, text_content, links, assets)
           VALUES ($1, $2, 'error', $3, NULL, NULL, NULL, NULL, '[]'::jsonb, '[]'::jsonb)`, [jobId, current, null]);
            }
        }
        await db_1.pool.query(`UPDATE crawl_jobs SET status = 'completed', pages_discovered = $2, pages_crawled = $3, completed_at = now(), updated_at = now() WHERE id = $1`, [jobId, discovered, crawled]);
        await db_1.pool.query(`UPDATE onboarding_sessions SET crawl_status = 'completed', updated_at = now() WHERE id = $1`, [sid]);
...
router.get("/status", async (req, res) => {
    try {
        ...
        if (!session_id)
            return res.status(400).json({ error: "session_id required" });
        const s = await db_1.pool.query(`SELECT id, customer_email, stripe_session_id, start_url, status, crawl_status, created_at, updated_at FROM onboarding_sessions WHERE id = $1 LIMIT 1`, [session_id]);
        if (!s.rows.length)
            return res.status(404).json({ error: "not found" });
        const j = await db_1.pool.query(`SELECT id, status, pages_discovered, pages_crawled, error, started_at, completed_at FROM crawl_jobs WHERE onboarding_session_id = $1 ORDER BY created_at DESC LIMIT 1`, [session_id]);
...
router.get("/pages", async (req, res) => {
    try {
        ...
        if (!session_id)
            return res.status(400).json({ error: "session_id required" });
        const j = await db_1.pool.query(`SELECT id FROM crawl_jobs WHERE onboarding_session_id = $1 ORDER BY created_at DESC LIMIT 1`, [session_id]);
        if (!j.rows.length)
            return res.json({ ok: true, pages: [] });
        const pages = await db_1.pool.query(`SELECT id, url, status, http_status, title, content_type, created_at FROM website_pages WHERE crawl_job_id = $1 ORDER BY created_at ASC LIMIT $2`, [j.rows[0].id, limit]);
...
router.get("/page-detail", async (req, res) => {
    try {
        ...
        if (!session_id || !pageUrl)
            return res.status(400).json({ error: "session_id and url required" });
        const j = await db_1.pool.query(`SELECT id FROM crawl_jobs WHERE onboarding_session_id = $1 ORDER BY created_at DESC LIMIT 1`, [session_id]);
        if (!j.rows.length)
            return res.status(404).json({ error: "job not found" });
        const row = await db_1.pool.query(`SELECT id, url, status, http_status, content_type, title, html, text_content, meta, links, assets, created_at FROM website_pages WHERE crawl_job_id = $1 AND lower(url) = lower($2) LIMIT 1`, [j.rows[0].id, pageUrl]);
...
router.get("/analyze-theme", async (req, res) => {
    try {
        ...
        if (!j.rows.length)
            return res.status(404).json({ error: "job not found" });
        const pages = await db_1.pool.query(`SELECT id, url, title, assets, meta FROM website_pages WHERE crawl_job_id = $1 ORDER BY created_at ASC LIMIT 20`, [j.rows[0].id]);
        ...
        const detected_stack = (() => {
            try {
                ...
                if (hints.includes("wp-content") || hints.includes("wordpress"))
                    return "WordPress";
                if (hints.includes("wix"))
                    return "Wix";
                if (hints.includes("shopify"))
                    return "Shopify";
                if (hints.includes("squarespace"))
                    return "Squarespace";
            }
            catch { }
        ...
        const theme = await db_1.pool.query(`INSERT INTO theme_analysis(onboarding_session_id, detected_stack, primary_color, secondary_color, font_family, layout_meta, score)
       VALUES ($1, $2, $3, $4, $5, $6::jsonb, $7) RETURNING *`, [session_id, detected_stack, primary_color, secondary_color, font_family, JSON.stringify({ css_count: uniqueCss.length }), 0.8]);
...
router.post("/integrations", async (req, res) => {
    try {
        ...
        if (!session_id || !type)
            return res.status(400).json({ error: "session_id and type required" });
        const ins = await db_1.pool.query(`INSERT INTO integration_credentials(onboarding_session_id, type, data) VALUES ($1, $2, $3::jsonb) RETURNING id`, [session_id, String(type).trim(), JSON.stringify(data || {})]);
...
router.get("/entry", async (req, res) => {
    try {
        ...
        let sessionId = null;
        if (sid) {
            const existing = await db_1.pool.query(`SELECT id FROM onboarding_sessions WHERE stripe_session_id = $1 LIMIT 1`, [sid]);
            sessionId = existing.rows[0]?.id || null;
        }
        if (!sessionId) {
            const ins = await db_1.pool.query(`INSERT INTO onboarding_sessions(customer_email, stripe_session_id, start_url, status, crawl_status) VALUES ($1, $2, $3, 'started', 'pending') RETURNING id`, [null, sid, start_url]);
            const job = await db_1.pool.query(`INSERT INTO crawl_jobs(onboarding_session_id, seed_url, status) VALUES ($1, $2, 'pending') RETURNING id`, [ins.rows[0].id, start_url || ""]);
