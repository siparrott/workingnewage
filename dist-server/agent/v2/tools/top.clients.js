...
const params = zod_1.z.object({
    metric: zod_1.z.enum(["revenue", "transactions", "lifetime_value", "recency"]).default("revenue").describe("How to rank clients"),
    limit: zod_1.z.number().int().min(1).max(100).default(20).optional().describe("Number of top clients to return"),
    startDate: zod_1.z.string().optional().describe("Only count activity after this date (YYYY-MM-DD)"),
    endDate: zod_1.z.string().optional().describe("Only count activity before this date (YYYY-MM-DD)"),
    minRevenue: zod_1.z.number().optional().describe("Minimum revenue threshold")
});
// Tool definition
const def = {
    name: "top_clients",
