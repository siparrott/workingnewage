...
const params = zod_1.z.object({
    campaignId: zod_1.z.string().optional().describe("Get analytics for a specific campaign ID"),
    period: zod_1.z.enum(["7d", "30d", "90d", "all"]).default("30d").optional().describe("Time period for analytics"),
    metric: zod_1.z.enum(["opens", "clicks", "bounces", "unsubscribes", "all"]).default("all").optional().describe("Specific metric to analyze")
});
const def = {
...
Returns: Comprehensive email marketing analytics`,
    ...
    handler: async (ctx, args) => {
      ...
      `;
            ...
            const topCampaigns = topResult.rows.map((row) => ({
                id: row.id,
                name: row.name,
                subject: row.subject,
