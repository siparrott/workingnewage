...
const params = zod_1.z.object({
    sessionId: zod_1.z.string().uuid().describe("Price wizard session ID containing the suggestions to activate"),
    suggestionIds: zod_1.z.array(zod_1.z.string().uuid()).optional()
        .describe("Specific suggestion IDs to activate (if omitted, activates all pending suggestions)"),
    priceAdjustments: zod_1.z.record(zod_1.z.string(), zod_1.z.number()).optional()
        .describe("Optional price adjustments as { suggestionId: newPrice }"),
    serviceCategory: zod_1.z.string().default("Photography").optional()
...
Returns: Summary of activated prices with created price list item IDs`,
    ...
    handler: async (ctx, args) => {
      ...
      `;
            const queryParams = [args.sessionId];
