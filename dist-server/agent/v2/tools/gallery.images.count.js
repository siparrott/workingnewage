...
const params = zod_1.z.object({
    galleryId: zod_1.z.string().optional().describe("Get image count for a specific gallery"),
    clientId: zod_1.z.string().optional().describe("Get image counts for a specific client's galleries")
});
const def = {
...
Returns: Image counts and gallery statistics`,
    ...
    handler: async (ctx, args) => {
        ...
        `;
                ...
                const totalImages = galleries.reduce((sum, g) => sum + g.image_count, 0);
                return {
                    client: client ? {
                        id: args.clientId,
                        name: client.name,
      ...
      `;
            ...
            const topGalleries = topResult.rows.map((row) => ({
                id: row.id,
                title: row.title,
                slug: row.slug,
