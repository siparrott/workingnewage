...
const params = zod_1.z.object({
    messageType: zod_1.z.enum(["email", "sms", "note", "any"]).default("any").optional().describe("Filter by message type"),
    status: zod_1.z.enum(["unread", "read", "replied", "sent", "delivered", "failed", "any"]).default("any").optional().describe("Filter by message status"),
    direction: zod_1.z.enum(["inbound", "outbound", "any"]).default("any").optional().describe("Filter by direction: inbound (received) or outbound (sent)"),
    clientId: zod_1.z.string().optional().describe("Filter by specific client ID"),
    search: zod_1.z.string().optional().describe("Search in subject, content, sender name or email"),
    startDate: zod_1.z.string().optional().describe("Filter messages after this date (YYYY-MM-DD)"),
    endDate: zod_1.z.string().optional().describe("Filter messages before this date (YYYY-MM-DD)"),
    unreadOnly: zod_1.z.boolean().optional().describe("If true, only show unread messages"),
    limit: zod_1.z.number().int().min(1).max(500).default(100).optional().describe("Maximum number of messages to return")
});
// Tool definition
const def = {
