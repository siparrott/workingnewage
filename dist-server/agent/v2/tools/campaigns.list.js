...
const params = zod_1.z.object({
    status: zod_1.z.enum(["draft", "scheduled", "sending", "sent", "paused", "archived", "any"]).default("any").optional().describe("Filter by campaign status"),
    type: zod_1.z.enum(["broadcast", "drip", "transactional", "any"]).default("any").optional().describe("Filter by campaign type"),
    search: zod_1.z.string().optional().describe("Search in campaign name or subject"),
    limit: zod_1.z.number().int().min(1).max(100).default(50).optional().describe("Maximum number of campaigns to return")
});
const def = {
    name: "campaigns_list",
