...
const params = zod_1.z.object({
    location: zod_1.z.string().min(2).describe("Geographic location to research (e.g., 'Wien', 'Salzburg', 'Graz')"),
    services: zod_1.z.array(zod_1.z.enum(["family", "wedding", "newborn", "portrait", "corporate", "event"]))
        .min(1)
        .describe("Photography service types to research pricing for"),
    maxCompetitors: zod_1.z.number().int().min(1).max(20).default(10).optional()
...
Returns: Price research results with competitor data, extracted prices, and AI-generated suggestions`,
    ...
    handler: async (ctx, args) => {
        try {
            ...
            const serviceBreakdown = suggestions.reduce((acc, s) => {
            ...
            return {
                ...
                top_prices: prices.slice(0, 10).map((p) => ({
