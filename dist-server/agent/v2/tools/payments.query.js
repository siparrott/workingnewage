...
const params = zod_1.z.object({
    invoiceId: zod_1.z.string().optional().describe("Filter payments for a specific invoice ID"),
    invoiceNumber: zod_1.z.string().optional().describe("Filter payments for a specific invoice number"),
    paymentMethod: zod_1.z.enum(["bank_transfer", "cash", "credit_card", "paypal", "stripe", "any"]).default("any").optional().describe("Filter by payment method"),
    startDate: zod_1.z.string().optional().describe("Filter payments after this date (YYYY-MM-DD)"),
    endDate: zod_1.z.string().optional().describe("Filter payments before this date (YYYY-MM-DD)"),
    minAmount: zod_1.z.number().optional().describe("Minimum payment amount"),
    maxAmount: zod_1.z.number().optional().describe("Maximum payment amount"),
    limit: zod_1.z.number().int().min(1).max(500).default(100).optional().describe("Maximum number of payments to return")
});
// Tool definition
const def = {
    name: "payments_query",
