...
const params = zod_1.z.object({
    ...
    discountType: zod_1.z.enum(["percentage", "fixed_amount", "any"]).default("any").optional().describe("Filter by discount type"),
    valid: zod_1.z.boolean().optional().describe("If true, only show currently valid coupons (within date range)"),
    search: zod_1.z.string().optional().describe("Search in coupon code, name, or description"),
    limit: zod_1.z.number().int().min(1).max(500).default(100).optional().describe("Maximum number of coupons to return")
});
// Tool definition
const def = {
    name: "coupons_list",
