...
const params = zod_1.z.object({
    startDate: zod_1.z.string().optional().describe("Start date for summary (YYYY-MM-DD)"),
    endDate: zod_1.z.string().optional().describe("End date for summary (YYYY-MM-DD)"),
    includeDetails: zod_1.z.boolean().default(false).optional().describe("Include detailed breakdown by client")
});
// Tool definition
const def = {
    ...
    handler: async (ctx, args) => {
      ...
      `;
        ...
        `;
                ...
                result.top_clients = clientResult.rows.map((row) => ({
                    id: row.id,
