...
exports.getRecommendedMode = getRecommendedMode;
exports.wouldRequireConfirm = wouldRequireConfirm;
exports.explainGuardrail = explainGuardrail;
 ...
 */
function enforce(ctx, tool, args) {
...
function checkScopes(ctx, tool) {
    ...
    if (missingScopes.length > 0) {
        ...
        throw new Types_1.AuthzError(tool.name, tool.authz, ctx.scopes);
    }
 ...
 */
function checkMode(ctx, tool, args) {
    ...
    if (ctx.mode === "auto_safe" && isRisky) {
        ...
        if (!confirmed) {
            ...
            throw new Types_1.ConfirmRequiredError(tool.name, args, `This action is ${tool.risk} risk and requires confirmation`);
        }
 ...
 */
function getRecommendedMode(userRole) {
    switch (userRole) {
        case "admin":
        case "owner":
 ...
 */
function wouldRequireConfirm(mode, risk) {
    if (mode === "auto_full")
        return false; // Never needs confirm
    if (mode === "read_only")
        return false; // Would be blocked, not confirmed
    if (mode === "auto_safe" && (risk === "medium" || risk === "high"))
        return true;
    return false;
}
/**
 ...
 */
function explainGuardrail(mode, risk, tool) {
