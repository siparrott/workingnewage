...
exports.listToolNames = listToolNames;
exports.listOpenAITools = listOpenAITools;
 ...
 */
function listOpenAITools(scopes) {
...
async function executeTool(ctx, toolName, args) {
    ...
    try {
        ...
        if (!parseResult.success) {
            throw new Types_1.ValidationError(toolName, parseResult.error);
        }
        const validatedArgs = parseResult.data;
        // 3. Check authorization and mode (may throw ConfirmRequiredError)
        (0, Guardrails_1.enforce)(ctx, tool, validatedArgs);
        // 4. Execute handler
        const result = await tool.handler(ctx, validatedArgs);
        // 5. Log success
        const duration = Date.now() - startTime;
        await (0, Audit_1.logToolCall)(ctx, {
            tool: toolName,
            args: validatedArgs,
            result,
            ok: true,
...
function zodToJsonSchema(schema) {
    ...
    if (schema instanceof zod_1.z.ZodObject) {
        ...
        for (const [key, value] of Object.entries(shape)) {
            ...
            if (!(value instanceof zod_1.z.ZodOptional) && !(value instanceof zod_1.z.ZodDefault)) {
                required.push(key);
            }
        }
        return {
